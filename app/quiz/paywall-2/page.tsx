...
export default function PaywallPage() {
  ...
  // Timer state (10 minutes countdown) - persisted in localStorage
  ...
  useEffect(() => {
    ...
    if (savedTimer) {
      ...
      // First visit - start with 10 minutes
      setTimerMinutes(9);
      setTimerSeconds(59);
    }
    setTimerInitialized(true);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f0] flex flex-col font-sans animate-fadeIn">
      {/* Header */}
      <header className="relative pt-6 sm:pt-8 pb-2 bg-[#f5f5f0] z-30">
      ...
      <footer data-footer className="fixed bottom-0 left-0 right-0 z-50 flex justify-center sm:pb-6 pointer-events-none">
        <div className="bg-white w-full sm:max-w-[500px] rounded-t-[32px] sm:rounded-[32px] px-4 pt-4 pb-2 sm:pb-4 shadow-[0_-10px_60px_rgba(0,0,0,0.08)] pointer-events-auto border-t border-gray-100 sm:border-none">
          ...
          {/* Promo Code Applied - Ticket Style */}
          <div className="bg-[#E8F5E0] rounded-2xl p-3 mb-3 overflow-visible">
            ...
            {/* Promo Code + Timer Row */}
            <div className="flex items-center gap-2">
              {/* Promo Code */}
              <div className="flex-1 bg-white rounded-xl border border-gray-200 p-2.5 flex items-center gap-2">
                <div className="w-5 h-5 rounded-full bg-[#6B9D47] flex items-center justify-center flex-shrink-0">
                  <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <span className="text-[#1a1a1a] font-medium text-sm">
                  {userName ? `${userName.slice(0, 5)}_${new Date().toLocaleString('en-US', { month: 'short' })}${new Date().getFullYear()}` : 'PROMO2025'}
                </span>
              </div>
              ...
              <div className="text-center px-3">
                <div className="flex items-center gap-1">
                  <span className="text-[#6B9D47] font-bold text-xl">{String(timerMinutes ?? 0).padStart(2, '0')}</span>
                  <span className="text-[#6B9D47] font-bold text-xl">:</span>
                  <span className="text-[#6B9D47] font-bold text-xl">{String(timerSeconds ?? 0).padStart(2, '0')}</span>
                </div>
