...
export function useVoiceRecorder(
  ...
  const startRecording = useCallback(async () => {
    try {
      setError(null);
      // ВСЕГДА обновляем базовый текст перед началом записи (даже если запись уже была)
      // Это нужно, чтобы если пользователь вручную удалил текст, он не восстановился
      if (getCurrentText) {
